...
def save_csr1_db(mac,interface,vlan,connx):
        ...
        query = "insert into p2p_details (MAC_addr,CSR1_if,CSR1_if_vlan) values ('%s','%s','%s')" % (mac,interface,vlan)
...
def save_mx104_db(interface,vlan,rpe,connx):
        ...
        query = "update p2p_details set edge_device='%s',203_010_if_vlan='%s',203_010_if='%s' where CSR1_if_vlan='%s'" % (rpe,vlan,interface,vlan)
	cursor_call(query,connx)
...
dev = Device(host='10.1.2.3', user='user', password='pwd' )
dev1 = Device(host='10.1.2.4', user='user', password='pwd' )
		...
		if re.match('ge-1/3/3', interface) is not None:
			...
			for item in opt_interface_csr1:
				...
				if re.match('.*' + interface, item.name) is not None:
					...
					vlan = re.search('Out\(swap \.(.*?)\)', item.vlan).group(1)
