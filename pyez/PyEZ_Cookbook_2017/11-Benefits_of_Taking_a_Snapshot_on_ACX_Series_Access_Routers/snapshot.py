...
filelog = path + "/log/" + device_ip + "_" + time.strftime("%Y-%m-%d_%H:%M:%S") + ".log"
...
if os.path.exists(filerun):
        print "-> There is another script instance targeting same device:",device_ip
	log("-> There is another script instance targeting same device")
	file_log.close()
        sys.exit(1)
...
if j_device.facts['model'] == "ACX2200" or j_device.facts['model'] == "ACX1100" or j_device.facts['model'] == "ACX2100":
...
try: 
	rsp = j_device.rpc.request_snapshot(slice='alternate',dev_timeout=600)
       ...
       print "--> (e) Error when snapshoting the device.You may need to replace device asap!", err
       log("--> (e) Error when snapshoting the device.You may need to replace device asap!")
...
### shows contents of dumpdates file ### START
