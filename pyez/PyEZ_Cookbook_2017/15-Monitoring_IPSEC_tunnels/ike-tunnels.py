...
def main():
                ...
                for idx, tunnel in enumerate (l3):
                               try:
                                               print ((idx + 1),' - ',tunnel['sa-vpn-name'],' - ',tunnel['sa-remote-gateway'],'- ',tunnel['ike-sa-index'],'- ',tunnel['sa-tunnel-index'],' - ', "\n\t",tunnel['sa-local-identity'],' - ',tunnel['sa-remote-identity'],'\n' )
                               except KeyError:
                                               print ((idx + 1),' - ',tunnel['sa-vpn-name'],' - ',tunnel['sa-remote-gateway'],'- ',tunnel['sa-tunnel-index'],' - ', '\n\t',tunnel['sa-local-identity'],' - ',tunnel['sa-remote-identity'],'\n' )
